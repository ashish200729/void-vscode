...
In all other cases, combine all edits to the same file into one \`edit_file\` call with multiple SEARCH/REPLACE blocks.`,
	...
	browser_open: {
		name: 'browser_open',
		description: `Open the browser panel and initialize a browser session for automation.

Use this tool FIRST before any browser automation to:
- Open the visible browser panel in VS Code
- Create and initialize the automation backend session
- Load an initial URL (defaults to Google if not specified)

IMPORTANT: You must call this tool before using any other browser tools (navigate, click, screenshot, etc.).

WORKFLOW PATTERN:
1. browser_open → Open browser (REQUIRED FIRST STEP)
2. browser_navigate/browser_click/etc. → Perform automation
3. browser_close → Close browser when done`,
		params: {
			url: { description: '(Optional) The initial URL to load. If not provided, defaults to https://www.google.com' }
		},
		example: `Open browser with Google homepage:
	<browser_open>
	<url>https://www.google.com</url>
	</browser_open>

Open browser with default URL:
...
Use this tool to load web pages for automation, data extraction, or testing.

IMPORTANT: After navigation, the page may take time to load. Use browser_wait_for_selector before interacting with elements to ensure they're ready.

WORKFLOW PATTERN:
1. browser_navigate → Load the page
2. browser_wait_for_selector → Wait for key elements
3. browser_click/browser_fill/browser_type → Interact with elements
4. browser_screenshot/browser_get_content → Capture results`,
		params: {
			url: { description: 'The full URL to navigate to (must include http:// or https://).' }
		},
		example: `Navigate to Google homepage:
	<browser_navigate>
	<url>https://www.google.com</url>
	</browser_navigate>`
	},
	...
	browser_screenshot: {
		...
		description: `Capture a screenshot of the current browser state as a base64-encoded PNG image.

Use this tool to:
- Verify visual layout and styling
- Debug UI issues
- Capture evidence of page state
- Check if elements rendered correctly

The screenshot captures the entire visible viewport of the browser.`,
		params: {},
...
- Search for specific text or elements
- Verify page loaded correctly

NOTE: For large pages, the HTML may be truncated. Use browser_get_accessibility_tree for a more structured view of interactive elements.`,
		params: {},
		example: `Get the HTML content of the current page:
	<browser_get_content></browser_get_content>`
	},

	browser_get_accessibility_tree: {
		name: 'browser_get_accessibility_tree',
		description: `Get the accessibility tree of the current page, showing all interactive elements with their selectors.

This is the BEST tool for discovering what elements are on the page and how to select them.

Use this tool to:
- Find CSS selectors for buttons, links, inputs, and other interactive elements
- Understand the page structure and hierarchy
- Discover element names, labels, and roles
- Identify form fields and their attributes

The accessibility tree is much more concise than full HTML and focuses on elements you can interact with.

RECOMMENDED WORKFLOW:
1. browser_navigate → Load page
2. browser_get_accessibility_tree → Find elements and their selectors
3. browser_click/browser_fill/browser_type → Use the selectors you found`,
		params: {},
		example: `Get the accessibility tree to find interactive elements:
	<browser_get_accessibility_tree></browser_get_accessibility_tree>`
	},
...
- By text content: Use browser_get_accessibility_tree to find the selector
- Combination: "form.login button[type='submit']"

BEST PRACTICE: Always use browser_wait_for_selector before clicking to ensure the element is ready.

COMMON ERRORS AND FIXES:
- "Element not found" → Use browser_get_accessibility_tree to find the correct selector
- "Element not visible" → The element may be hidden, check with browser_get_content
- "Element not interactable" → Wait for the page to load with browser_wait_for_selector`,
		params: {
			selector: { description: 'CSS selector for the element to click.' }
		},
	...
	browser_type: {
		name: 'browser_type',
		description: `Type text into an input element character by character, simulating keyboard input.

This tool types text slowly (character by character) to simulate human typing. For faster form filling, use browser_fill instead.

Use browser_type when:
- The input has JavaScript listeners that fire on each keystroke
- You need to trigger autocomplete or suggestions
- The form validates input in real-time

For simple form filling without these requirements, use browser_fill (much faster).

NOTE: This tool requires approval since it can submit forms.`,
		params: {
			selector: { description: 'CSS selector for the input element.' },
			text: { description: 'The text to type into the element.' },
			delay: { description: 'Optional. Milliseconds between keystrokes. Default is 0.' }
		},
		example: `Type into a search input with autocomplete:
	<browser_wait_for_selector>
	<selector>input[name='search']</selector>
	</browser_wait_for_selector>
	<browser_type>
	<selector>input[name='search']</selector>
	<text>AI automation tools</text>
	<delay>50</delay>
	</browser_type>`
	},
	...
	browser_fill: {
		name: 'browser_fill',
		description: `Fill a form field instantly by setting its value directly.

This is the FAST way to fill forms - it sets the value immediately without typing each character.

Use browser_fill when:
- Filling standard form fields (text inputs, textareas)
- Speed is important
- You don't need to trigger keystroke events

For inputs that require keystroke events (autocomplete, real-time validation), use browser_type instead.

NOTE: This tool requires approval since it can submit forms.`,
		params: {
			selector: { description: 'CSS selector for the form field.' },
			value: { description: 'The value to set in the field.' }
		},
...
- Home, End, PageUp, PageDown`,
		params: {
			key: { description: 'The key to press (e.g., "Enter", "Tab", "Escape").' }
		},
		example: `Submit a search by pressing Enter:
	<browser_fill>
	<selector>input[name='q']</selector>
	<value>AI automation</value>
	</browser_fill>
	<browser_press>
	<key>Enter</key>
	</browser_press>`
	...
	browser_hover: {
		name: 'browser_hover',
		description: `Hover the mouse over an element to reveal hidden content or tooltips.

Use this tool to:
- Reveal dropdown menus
- Show tooltips
- Trigger hover effects
- Display hidden navigation

NOTE: After hovering, you may need to wait briefly for animations to complete before clicking revealed elements.`,
		params: {
			selector: { description: 'CSS selector for the element to hover over.' }
		},
		example: `Hover over a menu to reveal submenu items:
	<browser_hover>
	<selector>nav .menu-item</selector>
	</browser_hover>
	<browser_wait_for_selector>
	<selector>nav .submenu</selector>
	</browser_wait_for_selector>
	<browser_click>
	<selector>nav .submenu .item</selector>
	</browser_click>`
	},
...
This is a CRITICAL tool for reliable browser automation. Always use it after navigation or before interacting with dynamic elements.

Use this tool to:
- Wait for pages to load after navigation
- Wait for dynamic content to appear
- Ensure elements are ready before clicking/typing
- Prevent "element not found" errors

RECOMMENDED WORKFLOW:
1. browser_navigate → Load page
2. browser_wait_for_selector → Wait for key element
3. browser_click/browser_fill/browser_type → Interact with element

Default timeout is 30 seconds. If the element doesn't appear within the timeout, an error will be returned.`,
		params: {
			selector: { description: 'CSS selector for the element to wait for.' },
			timeout: { description: 'Optional. Maximum time to wait in milliseconds. Default is 30000 (30 seconds).' }
		},
		example: `Wait for a search input to appear after navigation:
	<browser_navigate>
...
This is a POWERFUL tool that can execute arbitrary JavaScript. Use with caution.

Use this tool to:
- Extract complex data that requires computation
- Manipulate the page in ways not covered by other tools
- Call JavaScript functions exposed by the page
- Retrieve computed values or application state

SECURITY WARNING: This tool executes arbitrary JavaScript and requires user approval.

LIMITATIONS:
- Maximum script length: 50,000 characters
- Cannot access Node.js APIs (file system, process, etc.)
- Runs in the page context (can access page variables and functions)
- Return value must be JSON-serializable`,
		params: {
			script: { description: 'JavaScript code to execute. The result will be returned.' }
		},
		example: `Get the title and URL of the current page:
	<browser_evaluate>
	<script>
	({
	...
	browser_back: {
		name: 'browser_back',
		description: `Navigate backward in the browser history (equivalent to clicking the back button).

Use this tool to:
- Return to the previous page
- Navigate through multi-step workflows
- Undo navigation mistakes`,
		params: {},
	...
	browser_forward: {
		...
		description: `Navigate forward in the browser history (equivalent to clicking the forward button).

Use this tool after browser_back to move forward again.`,
		params: {},
		example: `Go forward to the next page:
	<browser_forward></browser_forward>`
	},

	browser_reload: {
		name: 'browser_reload',
		description: `Reload the current page (equivalent to pressing F5 or clicking the refresh button).
...
- Extract URL parameters or fragments
- Track navigation flow`,
	...
	browser_close: {
		name: 'browser_close',
		description: `Close the browser panel and terminate the browser automation session.

Use this tool to:
- Clean up resources after automation is complete
- Close the browser when no longer needed
- Reset browser state for a fresh start

IMPORTANT: After closing, you must call browser_open again to resume browser automation.

BEST PRACTICE: Always close the browser when your automation task is complete to free up system resources.`,
		params: {},
...
- open_persistent_terminal
- kill_persistent_terminal
- ALL BROWSER TOOLS (browser_navigate, browser_click, browser_type, browser_fill, browser_press, browser_hover, browser_screenshot, browser_get_content, browser_get_accessibility_tree, browser_wait_for_selector, browser_evaluate, browser_back, browser_forward, browser_reload, browser_get_current_url)
